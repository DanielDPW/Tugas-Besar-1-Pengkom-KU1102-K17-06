...
UserCode = 0 # UserCode adalah variabel yang akan diinput dengan kode produk yang ingin dibeli.
...
def Interface():
    ...
    os.system('cls')
    print("         +@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@-       ", end = '')
    print("Welcome to ROGA's Vending Machine")
    print("         *@                        :@:             @=       ", end = '')
...
while Start == False: # Saat nilai variabel Start itu false, maka program akan dimulai.
    ...
    while Cash > 0: # Karena variabel Cash tadi di awal adalah 1, maka loop ini akan berjalan. 
        Cash = Money(Cash) # Nilai Cash nanti akan kita assign dengan fungsi Money. Nilai Cash tidak akan pernah di bawah nol karena fungsi Money sudah memastikan bahwa variabelnya 0 dan ke atas.
    ...
    while Run: # Secara default, variabel Run bernilai true dan selama true akan meminta input kode Produk.
        ...
        Interface()
        Receipt()
        UserCode = Code(UserCode) # Input kode produk. Sama seperti fungsi Money, fungsi Code juga akan memastikan bahwa nilai UserCode tersebut tepat.
    ...
    if UserCode == 0 and Order == [0 for i in range(len(ID))]: # Namun jika pengguna tidak memesan apapun dan menekan 0, maka akan kembali ke tampilan awal.
        ...
        Help = "Type any input to confirm and type 0 to deny"
        Interface()
        Receipt()
